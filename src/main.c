...
// This is the basic use case scenario.
// I will try to comment this clearly so we can use it as an example of how to use
// the module.
static char * testfeGenerateReproduce() {
    // First, we initialise all the structs. We free helper data in this case,
    // because I use a global variable to keep the tests shorter.
    freeHelperData(&h);
    FEProperties p;

    // Fingerprints have a fixed size of 16 bytes.
    const size_t len = 16;

    // I initialise the fuzzy extractor so that it can reproduce up to
    // a Hamming error of 4 bits, with a reproduction error of 0.001.
    initFEProperties(&p, len, 4, 0.001);

    unsigned char fingerprint[len];     // Fingerprint obtained from an SRAM PUF
    unsigned char key[len];             // Key generated by feGenerate()
    unsigned char reproduced[len];      // Reproduced key. If key == reproduced, the fingerprint was valid.
    unsigned char reproduced2[len];
    int ret;

    // This loop was for testing purposes - I kept it in in order to make
    // multiple passes of the test possible.
    for (size_t i = 0; i < 1; i++)
    {
        // Let us assume a random fingerprint and ust fill the array with random bytes.
        randombytes_buf(fingerprint, sizeof(fingerprint));

        // The other fields will be overwritten by the functions - but I zero them anyway.
        memset(key, 0, len);
        memset(reproduced, 0, len);
        memset(reproduced2, 0, len);

        // Alright, let's enrol the fuzzy extractor. Give it the fingerprint,
        // and it will produce a key and helper data.
        ret = feGenerate(fingerprint, key, len, &h, &p);
        // If ret != 0, this unit test will fail.
        mu_assert("Error: feGenerate failed.", ret == 0);

        printHelperData(&h, false);

        // Test: identical fingerprint produces identical key
        // I pass the original fingerprint (!) to the reproduction function.
        // This must always pass. reproduced == key
    ...
    // Don't forget to free Helper data.
    freeHelperData(&h);
    return 0;
}

// This test checks if the fuzzy extractor can handle input with flipped bits.
// 2 bits are flipped: no problem, the FE can handle 4 bit flips.
// 13 bits flipped: will fail to reproduce.
